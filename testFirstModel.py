...
def main():
    ...
    model = keras.models.load_model('one_county_model')

    df = pd.read_csv('nueralNetworkData.csv')

    data = df[["Previous Cases", "Cases", "County Population Density", "Population", "Day Number"]]
    ...
    for i in range(100):
        ...
        sample = wholeRow.loc[:, wholeRow.columns != 'Cases']

        actual = wholeRow.loc[:, wholeRow.columns == 'Cases']

        actual_case = actual['Cases'].values[0]
